...
calendar = {
	...
	makeCalendar:function(year, month, day) {
		...
		for(var weekday=0; weekday<7; weekday++) this.wrt("<td align='center'>"+this.weekdays[weekday]+"</td>");
		this.wrt("</tr>");
		...
		for(var i=0;i<=5;i++) {
			...
			for(var j=0;j<7;j++) {
				...
				if(flag) {
					...
					this.wrt("<td align='center' class='days"+class_name+"'><a href='javascript:calendar.selectDate(\""+year+"\",\""+mon+"\",\""+w+"\")'>"+w+"</a></td>");
					d++;
				} else {
		...
		this.wrt("</table>");
		this.wrt("<input type='button' value='Ascunde' class='calendar-cancel' onclick='calendar.hideCalendar();' />");
